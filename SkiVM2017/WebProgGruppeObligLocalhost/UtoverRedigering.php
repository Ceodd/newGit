...
<html>
    ...
    <body>
        <nav class="navbar navbar-inverse">
            <div class="container-fluid">
                ...
                <ul class="nav navbar-nav">
                    ...
                    if (isset($_SESSION["verifisering"])) {
                        if ($_SESSION["verifisering"] == true) {
                            echo '<li class="active"><a href="UtoverRedigering.php">Rediger Utøvere</a></li>';
                        }
                    }
                    ?>
                    <?php
        ...
        <div class="col-centered">    
            <div class="col-md-8">
                <?php
                if (isset($_POST["utoverRegistrer"])) {
                    $nyutover = new utover();
                    $nyutover->set_uovelseid($_POST["regOvelseid"]);

                    $feilString = validerRegistrering($nyutover);
                    if ($feilString != null) {
                        echo "<font color=red><br/>Valideringsfeil:</font> <br/><font color=red>$feilString</font>";
                    } else {
                        $nyutover->registrer_utover();
                    }
                }
                ...
                $sql = "SELECT fornavn, etternavn, epost, adresse, telefonnummer, ovelseid FROM utover";

                $resultat = mysqli_query($db, $sql);

                if (!$resultat) {
                    trigger_error(mysqli_error($db));
                    echo "<br/> Feil, klarte ikke å registrere utover!";
                }
                ...
                $sql = "SELECT utoverid FROM utover";
                ...
                if (isset($_POST["slett"])) {
                    ...
                    if (!empty($_POST['check_list'])) {
                        foreach ($_POST['check_list'] as $check) {
                            $sqlSlett .= "DELETE FROM utover ";
                            $sqlSlett .= "WHERE utoverid=" . $utoverid[$check] . ";";
                        }
                    }
                    if ($sqlSlett == "") {
                ...
                if (isset($_POST["oppdater"])) {
                    for ($i = 0; $i < $_SESSION["utoverRadteller"]; $i++) {
                    ...
                    if ($uendret == $endret) {
                        ...
                        for ($i = 0; $i < $_SESSION["utoverRadteller"]; $i++) {
                            if ($uendret[$i] != $endret[$i]) {
                                $nyutover = new utover();
                                $feilString = validerEndring($nyutover, $endret, $i);
                                if ($feilString != null) {
                                    echo "<font color=red><br/>Valideringsfeil:</font> <br/><font color=red>$feilString</font>";
                                } else {
                                    $nyutover->set_uovelseid($endret[$i][5]);
                                    $nyutover->set_utoverid($utoverid[$i]);
                                    $nyutover->oppdater_utover();
                                }
                            }
                        }
                    }
                ...
                $_SESSION["utoverRadteller"] = $radteller;
             ...
             telefonnumerOK = valider_telefonnummer' . $i . '();
                ...
                <script type="text/javascript">
                    ...
                    function valider_fornavn() {
                        regEx = /^[a-zA-ZæøåÆØÅ .\-]{2,20}$/;
                        OK = regEx.test(document.utoverReg.fornavn.value);
                        if (!OK) {
                    ...
                    function valider_etternavn() {
                        regEx = /^[a-zA-ZæøåÆØÅ .\-]{2,20}$/;
                        OK = regEx.test(document.utoverReg.etternavn.value);
                        if (!OK) {
                    ...
                    function valider_epost() {
                        regEx = /^[a-zA-ZæøåÆØÅ0-9._-]+@[a-zA-ZæøåÆØÅ0-9.-]+\.[a-zA-Z]{2,20}$/;
                        OK = regEx.test(document.utoverReg.epost.value);
                        if (!OK) {
                    ...
                    function valider_adresse() {
                        regEx = /^[a-zA-ZæøåÆØÅ0-9 .\-]{2,20}$/;
                        OK = regEx.test(document.utoverReg.adresse.value);
                        if (!OK) {
                    ...
                    function valider_telefonnummer() {
                        regEx = /^[0-9]{8}$/;
                        OK = regEx.test(document.utoverReg.telefonnummer.value);
                        if (!OK) {
                ...
                <form action="" name="utoverReg" method="post" onsubmit="return valider_utreg()">
                    <table width="auto" class="table table-striped">
                        ...
                        <tr><td> <input type="text" name="fornavn" onchange="valider_fornavn()"> </td>
                            ...
                            <td> <input type="text" name="telefonnummer" onchange="valider_telefonnummer()" > </td>
                            ...
                            <td>
                                <select name="regOvelseid">
                                    ...
                                    <tr/>
                                    <tr>
                                        <td><input type="submit" value="Registrer" name="utoverRegistrer"/></td>
                                    </tr>
                    </table>
                </form>
